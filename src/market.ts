...
export class Market {
  ...
  public active(): Promise<Active> {
    return this.common.request('GET', `/v2${this.apiPrefix}/active`);
  }

  public tick(instrument: string, currency: string): Promise<Tick> {
    return this.common.request('GET', `${this.apiPrefix}/${instrument.toUpperCase()}/${currency.toUpperCase()}/tick`);
  }

  public orderbook(instrument: string, currency: string): Promise<OrderBook> {
    return this.common.request('GET', `${this.apiPrefix}/${instrument.toUpperCase()}/${currency.toUpperCase()}/orderbook`);
  }

  public async trades(instrument: string, currency: string, indexForward?: boolean, limit?: number, since?: number): Promise<Trades> {
    const qs = {
      indexForward,
      limit,
      since,
    ...
    const response = await this.common.request('GET', `/v2${this.apiPrefix}/${instrument.toUpperCase()}/${currency.toUpperCase()}/trades`, qs);
  ...
  public async historicTicks(instrument: string, currency: string, time: HistoricTicksTimeType, limit?: number, since?: number, indexForward?: boolean, sortForward?: boolean): Promise<any> {
    const qs = {
      limit,
      since,
      indexForward,
    ...
    const response = await this.common.request('GET', `/v2${this.apiPrefix}/${instrument.toUpperCase()}/${currency.toUpperCase()}/tickByTime/${time}`, qs);
